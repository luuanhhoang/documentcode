...
export const markOnboardingDone = async (_userId: string) => {
  return;
};
...
export const verifyResetOtp = async (email: string, otp: string) => {
  const normalizedEmail = normalizeEmail(email);
  const [rows] = await mysqlPool.query<Array<RowDataPacket & { otp: string }>>(
    "select otp from password_resets where email = ? and expires_at > now() limit 1",
    [normalizedEmail]
  );
...
export const resetPassword = async (email: string, otp: string, password: string) => {
  ...
  const [updated] = await mysqlPool.execute<ResultSetHeader>(
    "update users set password_hash = ? where email = ? and status <> 'DELETED'",

...
export const coupleImagePublicPath = (fileName: string) => `/api/media/couple-images/${fileName}`;

export const coupleImageFilePath = (fileName: string) => path.join(coupleImageDir, fileName);

export function extractCoupleImageFileName(url: string) {
  const match = url.match(/^\/api\/media\/couple-images\/([a-f0-9-]+\.(?:jpg|png|webp|gif))$/i);

  return match?.[1] ?? null;
}

export async function saveCoupleImage(userId: string, imageData: unknown) {

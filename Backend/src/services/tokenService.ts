...
const serializeAuthCookie = (value: string, maxAgeSeconds: number) => {
  const parts = [
...
export const createAuthCookie = (token: string) =>
  serializeAuthCookie(encodeURIComponent(token), authCookieMaxAgeSeconds);

export const clearAuthCookie = () => serializeAuthCookie("", 0);

export const getAccessTokenFromCookieHeader = (cookieHeader: string | undefined) => {
  if (!cookieHeader) {
    return null;
  }

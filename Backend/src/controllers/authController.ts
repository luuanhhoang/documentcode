...
import {
  ...
  resetPassword,
  verifyResetOtp
} from "../services/coupleAuthService.js";
...
const getRequestUserId = (req: Parameters<RequestHandler>[0]) => {
  if (req.userId) {
    return req.userId;
  }
...
export const register: RequestHandler = async (req, res, next) => {
  try {
    const { displayName, email, password, confirmPassword } = req.body as Record<string, string>;
...
export const markOffline: RequestHandler = async (req, res) => {
  const userId = getRequestUserId(req);
...
export const resetPasswordController: RequestHandler = async (req, res, next) => {
  try {
    const { email, otp, password, confirmPassword } = req.body as Record<string, string>;
